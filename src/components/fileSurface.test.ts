...
describe("kindOf", () => {
  ...
  test("neighbors are unchanged", () => {
    ...
    expect(kindOf("brief.docx")).toBe("document");
    expect(kindOf("legacy.doc")).toBe("document");
    expect(kindOf("draft.pages")).toBe("document");
    expect(kindOf("memo.rtf")).toBe("document");
