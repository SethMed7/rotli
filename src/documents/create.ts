...
function table(rows: string[][]): string {
  ...
  return `<w:tbl><w:tblPr><w:tblBorders><w:top w:val="single" w:sz="4" w:color="${rule}"/><w:left w:val="single" w:sz="4" w:color="${rule}"/><w:bottom w:val="single" w:sz="4" w:color="${rule}"/><w:right w:val="single" w:sz="4" w:color="${rule}"/><w:insideH w:val="single" w:sz="4" w:color="${rule}"/><w:insideV w:val="single" w:sz="4" w:color="${rule}"/></w:tblBorders></w:tblPr>${rows
    .map(
      (row) =>
        `<w:tr>${row.map((cell) => `<w:tc>${paragraph(cell)}<w:tcPr/></w:tc>`).join("")}</w:tr>`,
    )
    .join("")}</w:tbl>`;
}
...
function styleXml(): string {
  ...
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:styles xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">${[
    ...
    style("Title", "Title", theme.headingFont, theme.titleSizeHalfPoints, theme.headingColor, { bold: true, spacingAfter: 120 }),
    style("Subtitle", "Subtitle", theme.bodyFont, theme.subtitleSizeHalfPoints, theme.mutedColor, { spacingAfter: 280 }),
    style("Heading1", "heading 1", theme.headingFont, theme.heading1SizeHalfPoints, theme.headingColor, { bold: true, spacingBefore: 300, spacingAfter: 120 }),
    style("Heading2", "heading 2", theme.headingFont, theme.heading2SizeHalfPoints, theme.headingColor, { bold: true, spacingBefore: 260, spacingAfter: 100 }),
    style("Heading3", "heading 3", theme.headingFont, theme.heading3SizeHalfPoints, theme.headingColor, { bold: true, spacingBefore: 220, spacingAfter: 80 }),
  ].join("")}</w:styles>`;
}
