...
// .excalidraw on disk (the board store: the Mac corpus or the web vault) and
// retargets any open canvas tab to the new id so the board keeps showing.
...
export async function renameBoardItem(boardId: string, raw: string): Promise<void> {
  const name = raw.trim();
  if (!name) return;
  const meta = await boardStore().rename(boardId, name);
