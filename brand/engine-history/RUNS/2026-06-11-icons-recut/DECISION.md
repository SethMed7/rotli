...
| Tags | **tilted tag, rounded** (the maintainer's pick — v1 tag shape and the hash both rejected) | clay dot |
...
## Tags replacement (the maintainer, same day — two passes)

Pass 1: v1 tag shape rejected → hash offered. Pass 2: the maintainer picked the **tilted rounded tag with the
...
## Dark-ground color fixes (the maintainer, same day — "it is just colors")
...
**APPROVED — the maintainer, 2026-06-11 ("perfect, let's move forward and freeze").**
