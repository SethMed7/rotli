...
**Input:** Seth's pushback on r6: Voice also moves ahead of the Memory — it's just STT + TTS
...
1. **Dictate** (frame A) — ⌥⌘V (rebindable), system-wide: the cocoa pill is the only UI. Whisper
   ...
   Seth's explicit ask); clean text lands at the caret on the peach tint. **Cleanup is the default;
...
Voice module proper (listen + voice mode) arriving in the Chat/Voice wave. Seth to call.

## Round-7 calls for Seth
...
## Addendum — STT engine direction (Seth, same day)

Seth prefers no OpenAI lineage. Clarified: Whisper/whisper.cpp is MIT open source with zero OpenAI
...
**LOCKED (same day):** Seth's machine verified — Apple M4 Max, 64 GB RAM, 40-core GPU. Parakeet TDT
