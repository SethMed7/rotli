...
pub(crate) fn claim(window: &Window, position: DropPosition) -> bool {
    ...
    if debug {
        ...
        native_drag::debug_log(&format!("drop pasteboard types {types:?} → {lane:?}"));
        native_drag::debug_log(&format!("drop promise types readable here {promise_types:?}"));
    }
    ...
    if !receivers.is_empty() {
        if debug {
            native_drag::debug_log(&format!(
                "drop calling in {} file promise(s) → {}",
                receivers.len(),
                destination.display()
            ));
        }
    ...
    match write_image_bytes(&pasteboard, &destination) {
        Some(path) => {
            if debug {
                native_drag::debug_log(&format!("drop wrote pasteboard image bytes → {}", path.display()));
            }
        ...
        None => {
            ...
            if debug {
                native_drag::debug_log(&format!("drop had {lane:?} types but yielded no file"));
            }
            false
        }
    }
