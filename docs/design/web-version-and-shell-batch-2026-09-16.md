...
7. **Superseded 2026-09-22: a vault is required.** Items 5–6's browser
   vault and read-only copy are gone; every browser writes real files —
   Chromium directly, Zen/Firefox/Brave through Rotli Helper's vault lane —
   and nothing is usable until a vault is connected. The current contract is
   [`../architecture/web-vault-connection.md`](../architecture/web-vault-connection.md).
