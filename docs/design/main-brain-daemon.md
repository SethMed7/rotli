...
**Amendment (Seth, 2026-07-03 — feedback #15/#16):** Main is no longer populated *only* by hand. **Every new note auto-files into Main** the moment it's created (⌘N, the "+" menu, and the ⌘T / tab-strip "+" path), landing **inside the folder the user is working in** — an explicitly selected `main:<path>` folder, else the Main folder of the currently-active note — else at the Main root. This is still a pure id-manifest add (`addNoteToMainAt`, `mainTree.ts`); the physical file is untouched and the daemon-immunity above is unchanged. A `main:<path>` selection is a *view*, so it never routes physical creation (it'd hit a non-existent disk path) — the note is created via the normal route (memex `wiki/_inbox` staging when writable, else the local Inbox) and only its Main *slot* follows the folder. Wiring: `newNote`/`fileNewNoteIntoMain`/`newNoteInTab` in `keys/actions.ts`.
