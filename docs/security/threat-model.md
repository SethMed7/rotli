...
| A downloaded provider runtime is tampered with, or the agent inherits the host's Google identity | Registry-listed URL only, SHA-256 + size pinned in source and verified before extraction, byte-capped stream, install is a user action; the agent runs with a private `GEMINI_HOME` profile (0700, file storage), host `GOOGLE_*`/`GEMINI_*` variables stripped, `BROWSER` pinned to a no-op, and only Google's own loopback-redirect authorization URL is opened | Google controls the agent's own network traffic and account policy; the pinned release must be re-verified when the registry entry changes |
...
| A remote MCP credential grants the wrong role or survives replacement | Pair/start/unpair IPC is main-Settings-only; the generic webview Keychain commands cannot address the pairing bundle; independent random client/device tokens share only an opaque pair id; relay endpoints enforce role and exact live-poll token matching; regeneration stops the old generation before replacing the Keychain bundle; explicit unpair stops the connector and deletes the bundle; device token never crosses IPC | The newly generated client bearer intentionally crosses the owned Settings webview once for copying and then resides in the chosen cloud client; Rotli cannot delete the copy held by that client, but it has no matching connected device after unpair; compromise of either surface exposes ordinary non-secure workspace authority while Rotli is connected |
