...
| 16a | Remote relay connector | SAFE | every response is produced by the same workspace dispatcher and `read_for_ai`/write gates; the outbound transport adds no alternate corpus reads, starts only by explicit user action, disconnects on vault switch, uses separate client/device credentials, and logs no frames |
