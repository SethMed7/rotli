...
| CLI lanes (`claude` / `codex` / `cursor` / `antigravity`) | Anthropic/OpenAI/Cursor/Google through their official local clients or agents | transcript | native provider allowlist before binary lookup; binary allowlist (parity-pinned) and a model allowlist of static ids plus the ids the client itself reports (`provider_models.rs`: Claude's `initialize` control request under the lane's safe-mode/tools-off posture, Codex's own `models_cache.json`, `agent models`, Antigravity's `session/new` model option — no prompt, 20 s deadline, 1 MB cap, strict id shape), safe/tool-less/read-only argv or ACP protocol fields, secret scan; Cursor runs ACP Ask mode from an empty scratch workspace with client capabilities off and permission requests rejected; Antigravity runs Google's registry-listed ACP agent from a managed install with the same client posture (no fs/terminal capability, permissions rejected, native questions cancelled, no-tools override) and its own Google sign-in |
...
| Rotli Web page (`rotli.co/app/`) | `127.0.0.1` only (Rotli Helper) | vault file operations, chat prompts | `/app/` CSP `connect-src http://127.0.0.1:*`, `'self'` elsewhere (`check:web-privacy`); no-egress E2E (`e2e/web/rotli-web-privacy.spec.ts`); pairing token only in a URL fragment, stripped at boot |
| Rotli Helper vault lane (`helper_vault.rs`) | nothing (local file I/O) | the one user-chosen vault folder | loopback bind + origin allowlist + Host check + bearer; `resolve_beneath` confinement, symlinks skipped, `.git` never written, revision-gated atomic writes; root chosen by `--vault` or the OS picker, never the page ([`../architecture/web-vault-connection.md`](../architecture/web-vault-connection.md)) |
