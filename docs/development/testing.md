...
| `bun run capture:site` | Capture the hero's Warm Light workspace (`site/public/rotli-app-warm-light@3x.png`) and the Welcome folder's Tasks lesson in Main (`site/public/rotli-playground@3x.png`) at 3× density from a running local browser twin; the theme studio's `site/public/themes/` captures are a separate set it does not touch; `site/README.md` owns media regeneration and visual review |
