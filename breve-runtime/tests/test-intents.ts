 ...
 * conversations — including the exact exchanges that have failed for the maintainer — and
...
import {
  ...
  inboxScope,
  mailSearchAsk,
  saveAttachmentIntent,
  folderFromCaption,
  slugifyTopic,
  stripStepNarration,
  modelDirective,
} from "../scripts/intents";
...
function route(text: string): string {
  ...
  if (scheduleChangeGate(normalizeAmPm(text).toLowerCase(), schedulePairs(normalizeAmPm(text)).length))
    return "schedule";
  if (/^\/remind\s+/i.test(text) || (/\bremind me\b/i.test(text) && parseWhen(normalizeAmPm(text))))
    return "reminder";
...
const SCENARIOS: Array<[string, string, string]> = [
  ...
  [
    "incident2",
    "Give me the morning brief. I haven't seen it yet. Give me the audio morning brief. I will watch that then watch lunch.",
    "brief:morning:audio",
  ],
  ...
  [
    "incident3",
    "Give me an audio brief response on whether the chip selloff is overdone, do some research first",
    "audio-topic",
  ],
  // ── Brief vocabulary ──
  ...
  [
    "schedule",
    "I would like my lunch ones at noon, my morning at 7 a.m., and my night at 6 p.m.",
    "schedule",
  ],
  ...
  // ── Inbox triage vs send-email flow — the maintainer's vocabulary (his definitions 2026-06-12) ──
...
for (const [group, text, want] of SCENARIOS) {
  ...
  if (got !== want) {
    fail++;
    console.log(`FAIL [${group}] "${text}"\n  → ${got}, wanted ${want}`);
  }
}

// ── Saved-media capture (image/PDF filing) ───────────────────────────────────
let capChecks = 0;
const capFail = (m: string) => {
  fail++;
  console.log(`FAIL [capture] ${m}`);
};
const eq = (label: string, got: unknown, want: unknown) => {
  capChecks++;
  if (got !== want) capFail(`${label} → ${JSON.stringify(got)}, wanted ${JSON.stringify(want)}`);
};
...
eq(
  `intent(verbatim 06-17)`,
  saveAttachmentIntent("can you save this fold in my storage in folder related to coffee art"),
  true,
);
...
eq(
  `folder(verbatim 06-17)`,
  slugOf("Can you caldo save this fold in my storage in folder related to coffee art"),
  "coffee-art",
);
...
const LEAK =
  "I will start by listing the contents of the ~/breve directory. I will view the watchlist.md file to see how updates are managed. I will edit the watchlist to add both. Done. I've added Superintelligence to your watchlist under AI labs.";
eq(
  `strip(leak) recovers answer`,
  stripStepNarration(LEAK),
  "Done. I've added Superintelligence to your watchlist under AI labs.",
);
eq(
  `strip leaves clean answer alone`,
  stripStepNarration("Done. Added OpenCode to your watchlist."),
  "Done. Added OpenCode to your watchlist.",
);
eq(
  `strip keeps "I'll help…" opener`,
  stripStepNarration("I'll help you with that. Here's the rundown on Anthropic."),
  "I'll help you with that. Here's the rundown on Anthropic.",
);
eq(
  `strip keeps single "Let me check" opener`,
  stripStepNarration("Let me check the repo. The latest commit landed this morning and touches the gateway."),
  "Let me check the repo. The latest commit landed this morning and touches the gateway.",
);
eq(
  `strip preserves all-narration (no real answer)`,
  stripStepNarration("I will list the files. I will view the config."),
  "I will list the files. I will view the config.",
);
