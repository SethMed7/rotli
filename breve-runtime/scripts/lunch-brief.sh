...

{
  ...
  if ! brief_exists "$1"; then
    ...
    if brief_exists "$1"; then
      bun "$BREVE/scripts/notify.ts" "⚠ Your brief model ($BREVE_MODEL) was unavailable at lunch, so I generated the Pivot with $FALLBACK instead. On its way." || true
    else
      bun "$BREVE/scripts/notify.ts" "⚠ Couldn't generate your lunch Pivot — both $BREVE_MODEL and $FALLBACK look unavailable (Claude sub may be down). Reply \"brief\" to retry, or check the Mac." || true
