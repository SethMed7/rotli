...
run_model() { printf '%s\n' "$PROMPT" | caffeinate -i bun "$BREVE/scripts/local-brief.ts" "${TODAY}-lunch"; }
  ...
  if ! brief_exists "$1"; then
    ...
    if brief_exists "$1"; then
      ...
      bun "$BREVE/scripts/notify.ts" --idempotency-key "lunch-generation-failure-$TODAY" \
        "⚠ Couldn't generate your lunch Pivot with the on-device model. No cloud provider was contacted." || true
