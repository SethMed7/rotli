/**
 * BREVE time context — one place that knows what "today" and "now" mean for Seth.
...
export async function loadSettings(): Promise<Settings> {
  const s = (await Bun.file(SETTINGS_PATH).json().catch(() => null)) ?? {};
...
export function minutesNowIn(tz: string, d = new Date()): number {
  const [h, m] = d.toLocaleTimeString("en-GB", { timeZone: tz, hour: "2-digit", minute: "2-digit", hour12: false }).split(":");
  return parseInt(h) * 60 + parseInt(m);
}
/** The tz that currently governs Seth's day (travel-aware; auto-expires by date). */
...
export function resolveTz(raw: string): string | null {
  ...
  const MAP: Record<string, string> = {
    ...
    "uk": "Europe/London", "london": "Europe/London", "gmt": "Europe/London", "bst": "Europe/London",
    "cet": "Europe/Paris", "cest": "Europe/Paris", "paris": "Europe/Paris", "madrid": "Europe/Madrid", "berlin": "Europe/Berlin",
    "utc": "UTC", "tokyo": "Asia/Tokyo", "japan": "Asia/Tokyo", "india": "Asia/Kolkata", "dubai": "Asia/Dubai",
  };
  if (MAP[t]) return MAP[t];
  if (/^[A-Za-z]+\/[A-Za-z_]+$/.test(raw.trim())) {
    try { new Date().toLocaleDateString("en-CA", { timeZone: raw.trim() }); return raw.trim(); } catch {}
  }
  return null;
}
