 ...
 * daemon can show the maintainer the real thing (not just a path) before he CONFIRMs. Tight breve-only
...
export const plainName = (s: unknown): boolean =>
  typeof s === "string" && /^[\w.-]+$/.test(s) && !s.includes("..");
...
export function describeAction(a: PendingAction): string {
  switch (a.action) {
    case "install-launchd":
      return `install + load scheduled job ${a.plist}`;
    case "uninstall-launchd":
      return `unload + remove scheduled job ${a.label}`;
    case "chmod-script":
      return `make scripts/${a.script} executable`;
    case "run-script":
      return `run scripts/${a.script} ${Array.isArray(a.args) ? a.args.join(" ") : ""}`.trim();
    default:
...
export async function validateAction(breveRoot: string, input: unknown): Promise<string | null> {
  ...
  switch (a.action) {
    ...
    case "run-script": {
      ...
      if (!(await Bun.file(`${breveRoot}/scripts/${a.script}`).exists()))
        return `scripts/${a.script} not found`;
      if (
        a.action === "run-script" &&
        a.args !== undefined &&
        !(
          Array.isArray(a.args) &&
          a.args.every((x: unknown) => typeof x === "string" && /^[\w@.=:-]*$/.test(x))
        )
      )
...
// write+stage a script in one turn, so the maintainer sees that warning before approving.
