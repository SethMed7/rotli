 ...
 * behavior is sequence-testable (Seth keeps finding edges by talking to it; tests
...
export function briefQueueMatch(text: string): { note: string; url: string | null } | null {
  ...
  const queueVerb = /\b(talk about|talk to me about|mention|cover|include|add|put|note|jot|bring up|touch on|feature|discuss|write about|report on|keep covering)\b/;
  const briefWord = /\b(brief|briefs|briefing|pivot|nightcap|newsletter)\b/;
  const futureWord = /\b(tomorrow|tomorrows|tomorrow'?s|next|upcoming|future|going forward|each|every|daily|coming|the morning|over the next)\b/;
  const keepWatch = /\b(keep an eye on|keep watching|keep tracking|keep me (posted|updated|in the loop)|stay on top of|follow (this|that|it|the)|track (this|that|it|the)|monitor (this|that|it|the))\b/;
  ...
  const note = text.replace(/https?:\/\/\S+/g, "").replace(/\s+/g, " ").replace(/^[\s\-–—:]+/, "").trim();
...
export function briefAsk(text: string): { meal: Meal | null; format: BriefFormat; date?: string } | null {
  ...
  const wantsIt =
    /^\s*(can i (have|get) )?(the |my |an |a )?((audio|voice|spoken|morning|breakfast|daily|today'?s?|latest|lunch(time)?|mid-?day|noon|night(time)?|evening|tonight'?s?) )*(brief|pivot|nightcap)( please| pls)?( pdf| audio| view| text| file)?\s*[?.!]*\s*$/.test(t) ||
    /\b(send|give|get|share|forward|resend|show|attach|grab|pull up|have)\b.*\b(brief|pivot|nightcap)\b/.test(t) ||
    /\b(play|listen( to)?|hear|read)\b.*\b(brief|pivot|nightcap)\b/.test(t) ||
    /\b(audio|voice|spoken)\b.*\bbrief\b/.test(t) || /\bbrief\b.*\b(audio|voice|out loud)\b/.test(t) ||
    /\b(today'?s?|this morning'?s?|the morning|latest|daily|lunch|mid-?day|tonight'?s?|night|evening)\b.*\bbrief\b/.test(t) ||
    // meal word + brief word anywhere, as long as it's a request, not a question about content
    (!questiony && /\b(morning|breakfast|lunch(time)?|mid-?day|noon|night(time)?|evening|tonight'?s?)\b.*\b(brief|pivot|nightcap)\b/.test(t));
  if (!wantsIt) return null;
  // Seth: "I never want a picture" — view exists ONLY via the explicit /brief view command.
  const format: BriefFormat = /\b(pdf|file|document)\b/.test(t) ? "pdf"
    : /\btext\b/.test(t) ? "text"
    : "audio";
...
export function briefRegenMatch(text: string): { meal: Meal | null; model: ModelTier | null } | null {
  ...
  const regenVerb = /\b(re-?generate|re-?gen|re-?do|re-?make|re-?run|re-?build|re-?create|re-?write)\b/.test(t)
    || /\b(generate|make|do|build|run|create|give me|send me)\b[^.!?]*\b(again|another|a (?:new|fresh) one|fresh)\b/.test(t);
  if (!regenVerb) return null;
  const briefWord = /\b(brief|pivot|nightcap|newsletter)\b/.test(t);
  const mm = t.match(/\b(?:with|using|use|on|in|via|through|as)\s+(?:the\s+)?(gemini|gemma|local|haiku|sonnet|opus|fable|claude)\b/);
  ...
  if (mm) { const k = mm[1]; model = (k === "gemma" || k === "local") ? "local" : k === "claude" ? "sonnet" : (k as ModelTier); }
...
export function bareFollowup(text: string): BriefFormat | null {
  ...
  if (/^(the |my |as |in )?(audio|voice|spoken)( version| one| please| pls)?$/.test(t) || /^(out loud|say it|speak it|listen)$/.test(t)) return "audio";
...
export function wantsLastAsText(text: string): boolean {
  ...
  return /^(show|send|give( me)?|see|put)?\s*(me\s+)?(that|it|the (last|answer|reply))?\s*(as|in)\s+text\s*[?.!]*$/.test(t) ||
...
export function topicBriefMatch(text: string): { topic: string; deep: boolean } | null {
  ...
  if (!m) return null;
  return { topic: m[1].replace(/[?.!]+$/, "").trim(), deep: /\b(deep(er)?|detailed|thorough|in depth|full|research)\b/i.test(text) };
}
...
export function urlRequest(text: string): { mode: "summary" | "read"; url: string } | null {
  ...
  if (/\b(summar|tl;?dr|recap|gist|digest|what('s| is) (this|it)|key points)\b/.test(t)) return { mode: "summary", url };
...
export function watchIntentMatch(text: string): { url: string; condition: string | null } | null {
  ...
  const condition = text
    .replace(/https?:\/\/\S+/g, "")
    .replace(/\b(can you|please|watch|this page|this|and|keep an eye on|tell me (know )?(when|if)|let me (know )?(when|if))\b/gi, " ")
    .replace(/\s+/g, " ")
    .trim() || null;
 ...
 *  fine"). Honors what Seth says: if he genuinely commands a model, that tier runs (no
...
export function modelDirective(text: string): ModelTier | null {
  ...
  if (/\b(went with|going to (go|use)|gonna|maybe|probably|might|may|i'?d|should i|could i|would|later|eventually|some ?day|next time|for now)\b/.test(t)) return null;
  ...
  const lead = new RegExp(`^\\s*(?:please\\s+)?(?:can you\\s+|could you\\s+)?${pickVerb}\\s+(?:the\\s+)?${MODEL}\\b`);
  const anywhere = new RegExp(`\\b${pickVerb}\\s+(?:the\\s+)?${MODEL}\\b`);
  const onVia = new RegExp(`\\b(?:run|use|ask|do|send|put)\\b[^.!?]*?\\b(?:on|with|via|through)\\s+(?:the\\s+)?${MODEL}\\b`);
...
export function saveAttachmentIntent(caption: string): boolean {
  ...
  if (!/\b(save|keep|store|file|stash|archive|put|stick|add|drop|hold on ?to|hang on ?to)\b/.test(t)) return false;
  return /\b(this|it|that|these|them|image|images|photo|photos|pic|pics|picture|pictures|file|files|attachment|shot|screenshot|folder|storage)\b/.test(t);
}
...
export function folderFromCaption(caption: string): string | null {
  ...
  const strip = (s: string) =>
    s.replace(/\bfold(?:er|ers)?\b/g, " ")
      .replace(/\b(my|the|a|an|in|to|into|under|named|called|please|pls|thanks?|thx|storage|smstorage)\b/g, " ")
      .replace(/[^a-z0-9 ]+/g, " ")
      .replace(/\s+/g, " ")
      .trim();
  ...
  m = t.match(/\b(?:save|keep|store|file|put|stash)\b[^a-z0-9]*(?:this|it|that|the (?:image|photo|pic|picture|file|shot|screenshot))?\s*(?:in|to|into|under|as)\s+(.+)/);
  if (m) return strip(m[1]) || null;
  return null;
}
...
/** How to scope an inbox ask: a named account, "personal" group, "all", or ASK Seth.
...
export function inboxAsk(text: string): boolean {
  ...
  return /^\/inbox\b/.test(t.trim()) ||
    /\b(check|checked|anything|any|what'?s|new|unread|important|matter|waiting|look at|triage|catch up)\b/.test(t);
}
...
export function mailSearchAsk(text: string): { account: string | null; query: string } | null {
  const m = text.match(/\b(?:search|find|look)(?:\s+\w+){0,4}?\s+(?:e-?mails?|inbox(?:es)?|mail)\b.*?\b(?:for|about)\s+(.{3,})/i);
  if (!m) return null;
...
export function audioResearchAsk(text: string, asVoice = false): boolean {
  ...
  const researchy = /\b(deep(er)?|research|look into|review|analy[sz]|reasoning|investigate|dig into|thoughts on)\b/.test(t);
...
export function schedulePairs(text: string): Array<[Meal, number]> {
  ...
  const re = /\b(morning|breakfast|lunch(?:time)?|midday|noon brief|night(?:time)?|dinner|evening|nightcap)\b[^,;.]*?\b(?:at|by)\s+(noon|midnight|\d{1,2}(?::\d{2})?\s*(?:am|pm)?)/gi;
  let m;
  while ((m = re.exec(text))) {
    const meal: Meal = /lunch|midday/i.test(m[1]) ? "lunch" : /night|dinner|evening/i.test(m[1]) ? "night" : "morning";
    const mins = m[2].toLowerCase() === "noon" ? 720 : m[2].toLowerCase() === "midnight" ? 0 : parseClock(m[2]);
...
export function scheduleChangeGate(t: string, pairCount = 0): boolean {
  ...
  if (!/\b(moving forward|from now on|going forward|change|set|want|schedule|send|like)\b/.test(t)) return false;
...
export function parseWhen(t: string, now = new Date()): { due: number; rest: string } | null {
  let m = t.match(/\bin\s+(\d+(?:\.\d+)?)\s*(m(?:in(?:ute)?s?)?|h(?:(?:ou)?rs?)?|d(?:ays?)?|s(?:ec(?:ond)?s?)?)\b/i);
  if (m) {
  ...
  m = t.match(/\b(tomorrow|tonight|today)?\s*(?:at\s+)?(\d{1,2})(?::(\d{2}))?\s*(am|pm)?\b(?!\s*(m|h|d|min|hour|day))/i);
  if (m && (m[1] || m[4] || m[3])) {
    let h = parseInt(m[2]);
  ...
  if (m) { const d = new Date(now); d.setDate(d.getDate() + 1); d.setHours(9, 0, 0, 0); return { due: d.getTime(), rest: t.replace(m[0], "") }; }
  return null;
}
