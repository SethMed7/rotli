...
async function sendSignal(text: string) {
  if (DRY || !SIGNAL_ENABLED) { console.log("[watchers] delivery disabled — skip send"); return; }
  await sendSig(bot, owner, text);
}
...
for (const w of watchers) {
  ...
  if (!r.ok) {
    w.fails = (w.fails ?? 0) + 1;
    if (w.fails === 5) await sendSignal(`👁 Watcher #${w.id} (${w.url.slice(0, 60)}) has failed 5 checks in a row (${r.reason}) — still trying.`);
    keep.push(w);
    continue;
  }
  const text = r.text;
