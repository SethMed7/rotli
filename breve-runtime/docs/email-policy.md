...
*Status: live. This is the **policy + threat model** for Rotli's embedded brief feature. the maintainer's standing rules:
...
1. **Proton Mail Bridge** (requires a paid Proton plan — Mail Plus/Unlimited; the maintainer has one).
...
| Draft | OFF until the maintainer asks | IMAP `APPEND` to the Drafts folder — creates drafts with **zero send ability**. Shipped dark behind a flag in `settings.json` (`mail.drafting: false`). |
...
the maintainer's instructions. Concretely: inbox digests are produced by **local Gemma** with a
...
the maintainer's explicit ask + the existing CONFIRM gate. No auto-replies, no link-following
...
3. (later, flag-on) "Draft a reply to X saying …" → draft lands in Proton Drafts;
   the maintainer reviews + sends from the Proton app himself.
